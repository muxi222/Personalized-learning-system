"""
Retrieval hit-rate evaluator for Wzm.
...
1) Manual labels (preferred):
   --labels data/training/wzm/eval/retrieval_labels.jsonl
...
# Example: python training/modules/wzm/eval/retrieval_hit_rate.py --k 5
...
async def _retrieve_mcp(
    *,
    ...
    data = await client.search_questions(
        ...
        telemetry={"module": "wzm", "subject": subject, "user_id": user_id, "payload": {"eval": True}},
    )
...
async def _amain(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for subj in ("history", "geography", "other", "math", "english", "physics", "chemistry", "chinese", "biology", "politics", "economics"):
