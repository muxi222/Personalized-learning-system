#!/usr/bin/env python3
"""
Build Wzm GraphRAG KB (graph + hybrid retriever index).

Outputs:
- Graph: `data/training/wzm/graphrag/graph.json`
- Hybrid index (runtime compatible):
  - FAISS: `data/faiss/wzm/`
  - BM25:  `data/bm25/wzm/`
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default=str(_repo_root / "data" / "training" / "wzm" / "graphrag"),
    ...
    parser.add_argument(
        "--build-index",
        action="store_true",
        help="Also build the FAISS+BM25 hybrid index under data/faiss/wzm and data/bm25/wzm",
    )
    parser.add_argument(
        "--embedding-backend",
    ...
    parser.add_argument(
        "--vector-dir",
        default=str(_repo_root / "data" / "faiss" / "wzm"),
        help="FAISS store dir (should match backend module_vector_path for wzm)",
    )
    parser.add_argument(
        "--bm25-dir",
        default=str(_repo_root / "data" / "bm25" / "wzm"),
        help="BM25 store dir (should match backend module_bm25_path for wzm)",
    )
    ...
    if args.build_index:
        ...
        # - indexing parameters/config live under training/modules/wzm/embedding/...
        index_builder = _repo_root / "training" / "modules" / "wzm" / "embedding" / "scripts" / "build_index.py"
