"""
Build Wzm retriever index (FAISS + BM25) from SQLite questions.
...
def main():
    ...
    default_cfg = repo_root / "training" / "modules" / "wzm" / "embedding" / "configs" / "index.json"
    defaults = _load_defaults(default_cfg)

    parser = argparse.ArgumentParser(description="Wzm embedding/index builder (FAISS+BM25)")
    ...
    parser.add_argument("--vector-store-dir", default=defaults.get("vector_store_dir", "data/faiss/wzm"))
    parser.add_argument("--bm25-dir", default=defaults.get("bm25_dir", "data/bm25/wzm"))
    parser.add_argument(
        "--append",
        action="store_true",
