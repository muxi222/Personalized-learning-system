#!/usr/bin/env python3
"""
Build Tony retriever index (FAISS + BM25) from SQLite questions.
...
def main():
    ...
    default_cfg = repo_root / "training" / "modules" / "xmx" / "embedding" / "configs" / "index.json"
    defaults = _load_defaults(default_cfg)

    parser = argparse.ArgumentParser(description="XMX embedding/index builder (FAISS+BM25)")
    ...
    parser.add_argument("--vector-store-dir", default=defaults.get("vector_store_dir", "data/faiss/xmx"))
    parser.add_argument("--bm25-dir", default=defaults.get("bm25_dir", "data/bm25/xmx"))
    parser.add_argument(
        "--append",
        action="store_true",
    ...
    subjects = _parse_subjects(args.subjects, defaults.get("subjects", ["economics", "english", "other"]))
