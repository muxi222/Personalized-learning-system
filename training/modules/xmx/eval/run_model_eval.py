#!/usr/bin/env python3
"""
XMX model eval runner (OpenAI-compatible endpoint).
...
def main() -> int:
    parser = argparse.ArgumentParser(description="XMX model eval runner (vLLM)")
    parser.add_argument("--module", default="xmx")  # passed from pipeline.sh
    parser.add_argument("--api-base", default="http://127.0.0.1:8003/v1")
    parser.add_argument("--model", default="xmx-dpo")
    ...
    data_eval = repo_root / "data" / "training" / "xmx" / "eval"
    data_eval.mkdir(parents=True, exist_ok=True)
    eval_set = data_eval / "model_eval.jsonl"
    if not eval_set.exists():
        src = repo_root / "training" / "modules" / "xmx" / "eval" / "assets" / "model_eval_min.jsonl"
