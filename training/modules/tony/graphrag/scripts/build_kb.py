#!/usr/bin/env python3
"""
Build Tony GraphRAG KB (graph + hybrid retriever index).
...
def main():
    parser = argparse.ArgumentParser(description="Build Tony GraphRAG KB")
    parser.add_argument(
        "--sqlite",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(_repo_root / "data" / "training" / "tony" / "graphrag"),
    ...
    parser.add_argument(
        ...
        help="Subjects to include (tony: history geography other)",
    )
    ...
    parser.add_argument(
        "--build-index",
        action="store_true",
        help="Also build the FAISS+BM25 hybrid index under data/faiss/tony and data/bm25/tony",
    )
    parser.add_argument(
        "--embedding-backend",
    ...
    parser.add_argument(
        "--vector-dir",
        default=str(_repo_root / "data" / "faiss" / "tony"),
        help="FAISS store dir (should match backend module_vector_path for tony)",
    )
    parser.add_argument(
        "--bm25-dir",
        default=str(_repo_root / "data" / "bm25" / "tony"),
        help="BM25 store dir (should match backend module_bm25_path for tony)",
    )
    ...
    if args.build_index:
        ...
        # - indexing parameters/config live under training/modules/tony/embedding/...
        index_builder = _repo_root / "training" / "modules" / "tony" / "embedding" / "scripts" / "build_index.py"
