#!/usr/bin/env python3
"""
Prepare Tony SFT dataset from the production SQLite DB.
...
def main():
    parser = argparse.ArgumentParser(description="Prepare Tony SFT dataset")
    parser.add_argument(
        "--sqlite",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(_repo_root / "data" / "training" / "tony" / "datasets" / "sft"),
        help="Output base directory. Per-subject files are written to <output-dir>/<subject>/train.jsonl",
    )
    parser.add_argument(
        "--legacy-output",
        default=str(_repo_root / "data" / "training" / "tony" / "datasets" / "sft" / "train.jsonl"),
