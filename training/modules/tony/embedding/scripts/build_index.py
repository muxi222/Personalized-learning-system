...
def main():
    ...
    default_cfg = repo_root / "training" / "modules" / "tony" / "embedding" / "configs" / "index.json"
    defaults = _load_defaults(default_cfg)

    parser = argparse.ArgumentParser(description="Tony embedding/index builder (FAISS+BM25)")
    ...
    parser.add_argument("--vector-store-dir", default=defaults.get("vector_store_dir", "data/faiss/tony"))
    parser.add_argument("--bm25-dir", default=defaults.get("bm25_dir", "data/bm25/tony"))
    parser.add_argument(
        "--append",
        action="store_true",
