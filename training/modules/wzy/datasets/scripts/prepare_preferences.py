#!/usr/bin/env python3
"""
Prepare Wzy preference dataset from `feedbacks` table (human preference).
...
def _pick_default_demo_sft_paths(subjects: List[str]) -> List[Path]:
    ...
    for s in subjects:
        ...
        if not s:
            continue
        p = _repo_root / "data" / "training" / "wzy" / "datasets" / "sft" / s / "train.jsonl"
        if p.exists():
            out.append(p)
    legacy = _repo_root / "data" / "training" / "wzy" / "datasets" / "sft" / "train.jsonl"
    if legacy.exists():
        out.append(legacy)
    ...
    *,
    ...
    if not paths:
        raise FileNotFoundError(
            ...
            "Expected one of:\n"
            "- data/training/wzy/datasets/sft/<subject>/train.jsonl\n"
            "- data/training/wzy/datasets/sft/train.jsonl\n"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Prepare Wzy preference dataset (DPO/ORPO)")
    parser.add_argument(
        "--sqlite",
    ...
    parser.add_argument(
        "--output",
        default=str(_repo_root / "data" / "training" / "wzy" / "datasets" / "preference" / "train.jsonl"),
