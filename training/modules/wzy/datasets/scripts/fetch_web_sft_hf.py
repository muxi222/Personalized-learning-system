...
Wzy SFT JSONL rows, split by subject.
...
Output layout (default):
  data/training/wzy/datasets/sft_web/<subject>/train_web.jsonl
"""
...
AGIEVAL_GAOKAO_DATASETS: List[Tuple[str, str]] = [
    ...
    ("hails/agieval-gaokao-physics", "physics"),
]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch web (HF) subject datasets and convert to Wzy SFT JSONL")
    parser.add_argument(
        "--provider",
        default="hf-mmlu",
        help=(
            ...
            "- Or pass a raw HuggingFace dataset id, e.g. hails/agieval-gaokao-mathqa"
        ),
    )
    parser.add_argument(
        "--output-dir",
        default=str(_repo_root / "data" / "training" / "wzy" / "datasets" / "sft_web"),
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Optional subject filters (math/physics). Empty means all.",
    )
    ...
    try:
        if args.provider == "hf-mmlu":
            ...
            # New style: provider is the raw HF dataset id, e.g. hails/agieval-gaokao-mathqa
