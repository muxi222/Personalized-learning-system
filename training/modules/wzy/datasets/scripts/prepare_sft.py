#!/usr/bin/env python3
"""
Prepare Wzy SFT dataset from the production SQLite DB.
...
def main():
    parser = argparse.ArgumentParser(description="Prepare Wzy SFT dataset")
    parser.add_argument(
        "--sqlite",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(_repo_root / "data" / "training" / "wzy" / "datasets" / "sft"),
        help="Output base directory. Per-subject files are written to <output-dir>/<subject>/train.jsonl",
    )
    parser.add_argument(
        "--legacy-output",
        default=str(_repo_root / "data" / "training" / "wzy" / "datasets" / "sft" / "train.jsonl"),
