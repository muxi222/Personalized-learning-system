## Wzy Evaluation (MCP + Retrieval + UX Metrics)
...
```bash
python training/modules/wzy/eval/metrics_from_db.py --db ./data/sqlite/app.db
python training/modules/wzy/eval/retrieval_hit_rate.py --db ./data/sqlite/app.db --k 5 --max-questions 200
```
...
Run (requires `./deploy/scripts/pipeline.sh serve-model --module wzy up` already running):
...
- `data/training/wzy/eval/model_eval_report.json`

### Manual labels format (optional)
Create a file like `data/training/wzy/eval/retrieval_labels.jsonl`:
