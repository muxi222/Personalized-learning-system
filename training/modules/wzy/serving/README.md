## Wzy 模型部署（vLLM / 工业常见方案）

这里提供 **可落地的部署骨架**：本地 `docker compose`（开发/验收） + K8s（生产/集群化）。

### 目录约定（产物放 data/）

- **LoRA/SFT/DPO 产物**：`data/training/wzy/checkpoints/`
- **合并后的可推理模型（可选）**：`data/training/wzy/models/merged/`
- **推理服务配置**：`data/training/wzy/serving/`
...
- 基座：HF Hub `OpenPipe/Qwen3-14B-Instruct`
   - LoRA：`data/training/wzy/checkpoints/sft_lora`
3. 启动：

```bash
docker compose -f training/modules/wzy/serving/docker-compose.vllm.yml up -d
```
...
`training/modules/wzy/serving/k8s/` 提供基础清单：
- Deployment + Service
