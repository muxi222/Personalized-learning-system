...
async def create_question(
    ...
    async def process_async():
        from backend.modules.wzm.agents.intake.question_intake_agent import QuestionIntakeAgent
    ...
    try:
        ...
        if input_type == "image":
            ...
            async def process_image():
                from backend.modules.wzm.agents.intake.question_intake_ocr_agents import QuestionIntakeOCRAgent
...
async def create_question_with_image_old(
    ...
    try:
        ...
        async def process_with_ocr():
            ...
            try:
                # 1. OCR识别
                from backend.modules.wzm.config import settings as wzm_settings
                ocr_service = get_gemini_ocr_service(wzm_settings)
...
async def get_suggested_question_answer(
    ...
    try:
        from backend.modules.wzm.agents.intake.question_intake_ocr_agents import call_router_llm_json
    except Exception as e:
        raise HTTPException(status_code=501, detail=f"TODO: Suggested answer generator not available: {e}")

    model = os.getenv("WZM_PRACTICE_ANSWER_MODEL") or os.getenv("WZM_TEXT_MODEL") or None
...
async def delete_question(
    ...
    try:
        ...
        logger.warning(f"[wzm] cleanup orphan question image failed: {e}")
...
async def reanalyze_question(
    ...
    async def reanalyze_async():
        from backend.modules.wzm.agents.intake.question_intake_agent import analyze_error, update_result
