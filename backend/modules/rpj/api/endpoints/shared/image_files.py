"""RPJ - Image Files API（学生实现版 / Stub）
...
async def download_image_file(image_id: int = Path(...), db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    """TODO(student): 图片文件入口。"""
    raise HTTPException(status_code=501, detail="Not Implemented: download_image_file")

@router.post("/upload")
async def upload_image_file(file: UploadFile = File(...), db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    """TODO(student): 图片上传入口。"""
    raise HTTPException(status_code=501, detail="Not Implemented: upload_image_file")

@router.get("/stats/usage")
async def get_image_usage_stats(page: int = Query(1, ge=1), db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    """TODO(student): 图片文件入口。"""
    raise HTTPException(status_code=501, detail="Not Implemented: get_image_usage_stats")
