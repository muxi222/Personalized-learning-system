"""
OCR批改API (RPJ模块 - 完整实现版本)

本文件为RPJ模块的OCR批改功能完整实现。
...
from backend.modules.rpj.api.deps import get_current_user_id, get_optional_user_id
...
from backend.core.db.models import User, ImageFileTypeEnum, ExamCorrection, Question, QuestionSourceEnum
...
class OCRAnalysisResponse:
    ...
    def __init__(
        ...
        self.corrected_image_url = corrected_image_url
    ...
    def dict(self):
        ...
        return {
            ...
            "duplicate_message": self.duplicate_message,
        }
...
async def analyze_exam_image(
    ...
    # 检查是否为同一用户的重复图片
    if existing_image and existing_image.user_id == current_user_id:
        is_duplicate = True
        duplicate_message = "检测到您之前已上传过相同的图片，系统将复用已有图片进行分析"
        logger.info(f"Duplicate image detected for user {current_user_id}: hash={file_hash[:16]}...")
    
    # 生成task_id用于后续处理
    task_id = str(uuid.uuid4())
    
    # 获取当前用户对象
    current_user = await crud_user.get_user(db, current_user_id)
    if not current_user:
    ...
    try:
        ...
        for q in result.questions:
            if not q.is_correct:  # 只为错题创建记录
                ...
                question_filename = f"{task_id}_q{q.question_number}.{file_ext if 'file_ext' in locals() else 'jpg'}"
                question_image_path = os.path.join(
                    get_user_upload_dir(UPLOAD_DIR, current_user, "questions", subject_en),
                    question_filename
                )
                os.makedirs(os.path.dirname(question_image_path), exist_ok=True)
        ...
        logger.info(
            f"Saved exam correction {exam_correction.id} for user {current_user_id}, "
...
async def get_user_image(
    ...
    # 验证学科
    validate_subject(subject.lower())
    ...
    if current_user_id is not None:
        ...
        # 没有token的情况
        logger.warning(f"Image access without authentication: user_id={user_id}, file={filename}")
    ...
    if file_type not in ["corrections", "questions"]:
        raise HTTPException(status_code=400, detail="file_type必须是'corrections'或'questions'")
...
async def get_correction_image(
    ...
    if image_type == "original":
        if not correction.original_image:
        ...
        if not correction.corrected_image:
    ...
    if os.path.isabs(file_path):
        ...
        if "data/uploads" in normalized:
            ...
            if "data/uploads/" in normalized:
                rel_part = normalized.split("data/uploads/", 1)[-1]
    ...
    if not found_path:
        ...
        # 获取学科字符串
        subject_str = correction.subject.value if hasattr(correction.subject, 'value') else str(correction.subject)
        possible_dirs = [
            os.path.join(upload_base, user_dir_name, "corrections", subject_str),
            os.path.join(upload_base, user_dir_name, "corrections"),
        ]
        ...
        if not found_path:
            logger.error(f"Image file not found. Stored path: {original_stored_path}, image_id={image_file.id}")
...
async def batch_analyze_images(
    ...
    for file in files:
        try:
            # 这里应该复用单张分析逻辑
            # 为简化，这里只记录文件名，实际应异步处理
            content = await file.read()
            file_hash = calculate_file_hash(content)[:16]
...
async def get_correction_detail(
    ...
    获取批注详情
    """
    correction = await crud_exam_correction.get_exam_correction(db, correction_id, current_user_id)
    if not correction:
        raise HTTPException(status_code=404, detail="批注记录不存在")
    ...
    return {
        ...
        "correction": {
            ...
            "original_image_url": generate_correction_image_url(correction.id, "original", correction.subject.value) if hasattr(correction.subject, 'value') else None,
            "corrected_image_url": generate_correction_image_url(correction.id, "corrected", correction.subject.value) if hasattr(correction.subject, 'value') else None,
            "questions": correction.questions_detail,
        }
    }
...
async def list_corrections(
    ...
    corrections, total = await crud_exam_correction.get_user_corrections(
        db, current_user_id, subject, page, page_size
    )
