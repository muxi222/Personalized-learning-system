...
def ensure_text_format(text: str) -> str:
    """
    确保文本格式正确，特别是对于文科类题目
...
def build_fallback_question_image_url(question_id: int, image_index: int) -> str:
    """
    向后兼容：如果无法定位 image_files.id，则退回  模块按 question_id+index 取图。
    """
    base = _public_base()
...
async def call_router_llm_json_with_retry(
    ...
    for attempt in range(max_retries):
        try:
            # 导入原始函数
            from backend.modules.rpj.agents.intake.question_intake_ocr_agent import call_router_llm_json
...
async def create_question(
    ...
    async def process_async():
        from backend.modules.rpj.agents.intake.question_intake_agent import QuestionIntakeAgent
...
async def process_image_intake_with_optimized_prompt(
    ...
    核心原则：只有AI分析成功才算成功，否则都是失败
    """
    from backend.modules.rpj.agents.intake.question_intake_ocr_agent import QuestionIntakeOCRAgent
    from backend.core.db.session import async_session_maker
    from backend.core.crud import crud_task
    
    # 异步的失败标记函数
    async def mark_as_failed(error_msg: str, error_type: str = "unknown") -> Dict[str, Any]:
        """标记任务为失败"""
        logger.error(f"图片录入任务 {task_id} 失败: {error_msg}")
        try:
            async with async_session_maker() as session:
                await crud_task.fail_task(session, task_id, error_msg)
                await session.commit()
        except Exception as task_error:
            logger.error(f"更新任务状态失败: {task_error}", exc_info=True)
    ...
    # 第一轮：尝试主reasoner_agent（完整AI分析）
    try:
        agent = QuestionIntakeOCRAgent()
        logger.info(
            f"[rpj/questions/ocr] task_id={task_id} user_id={user_id} "
            f"开始完整AI分析，学科={subject}，难度={difficulty}"
        )
        
        # 设置环境变量，使用优化的reasoner_agent
        os.environ["RPJ_REASONER_MAX_TOKENS"] = "2048"
        os.environ["RPJ_DEEP_ENRICH_MAX_ITEMS"] = "2"
        os.environ["RPJ_SIMPLIFIED_MODE"] = "optimized"
        ...
        if result.get("success"):
            logger.info(f"图片录入任务 {task_id} 完整AI分析成功")
            return result
        ...
        if errors:
            if error_type == "timeout":
                return await mark_as_failed("AI分析超时：处理时间过长。请稍后重试。", "timeout")
            elif error_type == "rate_limit":
                return await mark_as_failed("请求过于频繁：请等待1分钟后再试。", "rate_limit")
            elif error_type == "service_unavailable":
                return await mark_as_failed("AI服务暂时不可用：请稍后重试。", "service_unavailable")
            elif error_type == "network_error":
                return await mark_as_failed("网络连接问题：请检查网络后重试。", "network_error")
            elif error_type == "all_failed":
                return await mark_as_failed("AI分析失败：可能是题目内容过于复杂。请尝试分开上传每道题目。", "all_failed")
            else:
                error_msg = str(errors[0]) if errors else "AI分析失败"
                return await mark_as_failed(error_msg, error_type)
        
        # 如果没有明确的错误信息，返回通用失败
        return await mark_as_failed("AI分析失败，无法完成错题录入", "unknown")
        
    except Exception as e:
        error_msg = str(e)
        logger.error(f"图片录入任务 {task_id} AI分析过程异常: {e}", exc_info=True)
        
        # 根据异常类型返回相应的错误
        if "ConnectError" in error_msg or "connection" in error_msg.lower() or "All connection attempts failed" in error_msg:
            return await mark_as_failed("网络连接失败：无法连接到AI服务。请检查网络连接后重试。", "network_error")
        elif "Timeout" in error_msg or "timeout" in error_msg.lower():
            return await mark_as_failed("请求超时：AI服务响应时间过长。请稍后重试。", "timeout")
        elif "rate limit" in error_msg.lower() or "429" in error_msg:
            return await mark_as_failed("请求过于频繁：请等待1分钟后再试。", "rate_limit")
        elif "service unavailable" in error_msg.lower() or "503" in error_msg:
            return await mark_as_failed("AI服务暂时不可用：请稍后重试。", "service_unavailable")
        else:
            return await mark_as_failed(f"系统错误：{str(e)[:100]}", "system_error")
...
async def create_question_intake_ocr(
    ...
    try:
        ...
        logger.info(
            f"[rpj/questions/ocr] intake request accepted task_id={task_id} user_id={user_id} "
        ...
        if input_type == "image":
            ...
            logger.info(
                f"[rpj/questions/ocr] task_id={task_id} user_id={user_id} image read bytes={len(content)} "
            ...
            logger.info(
                f"[rpj/questions/ocr] task_id={task_id} user_id={user_id} saved new image "
                ...
                try:
                    ...
                    if result.get("success"):
                        if result.get("mode") == "degraded":
                            logger.warning(
                                f"[rpj/questions/ocr] task_id={task_id} 降级处理成功，警告: {result.get('warning')}"
                            )
                        else:
                            logger.info(
                                f"[rpj/questions/ocr] task_id={task_id} 标准处理成功"
                            )
                    else:
                        logger.error(
                            f"[rpj/questions/ocr] task_id={task_id} 处理失败: {result.get('errors')}"
                        )
                        
                        # 更新任务状态为失败（如果process_image_intake_with_optimized_prompt没有更新）
                        if not result.get("task_updated", False):
                            async with async_session_maker() as session:
                                errors = result.get("errors", [])
                                error_msg = "; ".join([str(e) for e in errors if e]) or "处理失败"
                                # 修复：去掉 result 参数
                    ...
                    logger.error(f"图片录入任务 {task_id} 异步处理失败: {e}", exc_info=True)
                    try:
                        async with async_session_maker() as session:
                            # 修复：去掉 result 参数
                            await crud_task.fail_task(
            ...
            async def process_text():
                ...
                try:
                    agent = QuestionIntakeOCRAgent()
                    logger.info(
                        f"[rpj/questions/ocr] task_id={task_id} user_id={user_id} start agent.process(type=text) subject={subject} diff={difficulty} "
                    ...
                    logger.info(
                        f"[rpj/questions/ocr] task_id={task_id} user_id={user_id} agent.process done duration_ms={duration_ms} success={bool(result.get('success'))}"
                    )
        ...
        logger.info(f"[rpj/questions/ocr] 任务已创建 task_id={task_id} user_id={user_id} type={input_type}")
        ...
        return TaskResponse(
            ...
            details={
                ...
                "difficulty": difficulty,
                "input_type": input_type,
                "estimated_time": "约30-60秒" if input_type == "image" else "约10-20秒"
            }
        )
        ...
        logger.error(f"处理录入请求失败: {e}", exc_info=True)
        
        # 提供具体的错误信息
        error_msg = "处理失败"
        if "存储空间" in str(e) or "disk full" in str(e).lower():
            error_msg = "服务器存储空间不足，请联系管理员。"
        elif "权限" in str(e) or "permission" in str(e).lower():
            error_msg = "文件权限错误，请联系管理员。"
        elif "格式" in str(e) or "format" in str(e).lower():
            error_msg = "文件格式错误，请检查上传的文件。"
        
        raise HTTPException(status_code=500, detail=f"{error_msg}: {str(e)[:100]}")
...
async def create_question_with_image_old(
    ...
    try:
        ...
        async def process_with_ocr():
            ...
            try:
                # 1. OCR识别
                from backend.modules.rpj.config import settings as rpj_settings
                ocr_service = get_gemini_ocr_service(rpj_settings)
                ...
                # 根据不同学科使用不同的OCR提示词
                if subject_en in ["chinese", "语文"]:
                    ocr_prompt = """你是一个专业的语文学习助手。请识别图片中的错题，提取以下信息（如果图片中没有某项信息，请留空）：

1. **题目内容**: 完整的题目描述（文言文、现代文、诗歌等）
2. **学生答案**: 学生写的答案（如果有）
3. **正确答案**: 正确的答案或参考答案（如果有标注）
4. **题目类型**: 选择题/填空题/阅读理解/作文/文言文翻译/诗词鉴赏等
5. **知识点**: 涉及的知识点（如：古诗词默写、文言文实词、现代文阅读技巧等）
6. **错因分析**: 学生的错误类型（如：字词错误、理解偏差、答题不规范等）
...
如果图片模糊或无法识别，请说明原因。"""
                ...
                elif subject_en in ["english", "英语"]:
                    ocr_prompt = """You are a professional English learning assistant. Please identify the wrong question in the image and extract the following information (leave blank if not available):

1. **Question content**: Complete description of the question (reading comprehension, grammar, vocabulary, etc.)
2. **Student's answer**: The answer written by the student (if any)
3. **Correct answer**: The correct answer or reference answer (if marked)
4. **Question type**: Multiple choice/Blank filling/Reading comprehension/Writing/Translation, etc.
5. **Knowledge points**: Related knowledge points (e.g., grammar tense, vocabulary usage, reading skills, etc.)
6. **Error analysis**: Type of student error (e.g., grammatical error, vocabulary misuse, comprehension mistake, etc.)
...
If the image is blurry or unreadable, please explain why."""
                
                elif subject_en in ["moral_education", "道法"]:
                    ocr_prompt = """你是一个专业的道德与法治学习助手。请识别图片中的错题，提取以下信息（如果图片中没有某项信息，请留空）：

1. **题目内容**: 完整的题目描述（选择题、简答题、材料分析题等）
2. **学生答案**: 学生写的答案（如果有）
3. **正确答案**: 正确的答案或参考答案（如果有标注）
4. **题目类型**: 选择题/判断题/简答题/材料分析题/案例分析题等
5. **知识点**: 涉及的知识点（如：法律常识、道德规范、社会公德等）
6. **核心素养**: 题目考查的核心素养（如：法治意识、道德认知、社会责任等）
...
如果图片模糊或无法识别，请说明原因。"""
                    ...
                    # 默认提示词（如果传入其他学科，使用通用提示词）
                    ocr_prompt = """你是一个专业的学习助手。请识别图片中的错题，提取以下信息（如果图片中没有某项信息，请留空）：
...
async def get_question(
    ...
    # 确保文本格式字段正确处理
    if resp.explanation and isinstance(resp.explanation, str):
        resp.explanation = ensure_text_format(resp.explanation)
    if resp.error_analysis and isinstance(resp.error_analysis, str):
        resp.error_analysis = ensure_text_format(resp.error_analysis)
    if resp.student_answer and isinstance(resp.student_answer, str):
        resp.student_answer = ensure_text_format(resp.student_answer)
    if resp.correct_answer and isinstance(resp.correct_answer, str):
        resp.correct_answer = ensure_text_format(resp.correct_answer)
    
    # 处理举一反三题目中的答案格式
    for sq in resp.suggested_questions:
        if sq.answer and isinstance(sq.answer, str):
            sq.answer = ensure_text_format(sq.answer)
        if sq.explanation and isinstance(sq.explanation, str):
            sq.explanation = ensure_text_format(sq.explanation)
...
async def get_suggested_question_answer(
    ...
    """
    RPJ模块: lazily generate/fill the answer for a '举一反三'练习题。
    ...
    # Generate answer via router LLM (with RPJ tracing/logging)
    model = os.getenv("RPJ_PRACTICE_ANSWER_MODEL") or os.getenv("RPJ_TEXT_MODEL") or None
    trace_id = f"sq_answer_q{question_id}_i{index}_{uuid.uuid4().hex[:8]}"
    
    # 根据学科类型优化prompt
    subject = getattr(resp, 'subject', '')
    
    if subject == '语文':
        prompt = f"""
请作为语文老师，为以下练习题提供参考答案和解题思路：
...
{{
  "answer": "答案内容",
  "explanation": "详细的解题思路和步骤"
}}
""".strip()
    elif subject == '英语':
        prompt = f"""
Please act as an English teacher and provide reference answers and solution ideas for the following exercise:

Question: {sq.content}

Requirements:
1. The answer should be clear, complete, and grammatically correct
2. For reading comprehension, provide analysis of key points
3. For grammar questions, explain the rules
4. For writing tasks, provide writing outlines or sample structures

Please return in the following JSON format:
{{
  "answer": "Answer content",
  "explanation": "Detailed solution ideas and steps"
}}
""".strip()
    elif subject == '道法':
        prompt = f"""
请作为道德与法治老师，为以下练习题提供参考答案和解题思路：
...
""".strip()
    ...
    if not isinstance(answer, str) or not answer.strip():
        ...
        for key in ["答案", "solution", "result", "answer"]:
    ...
    # 确保答案格式正确
    answer = ensure_text_format(answer.strip())
    
    if explanation is not None and not isinstance(explanation, str):
        explanation = None
    elif explanation is not None:
        explanation = ensure_text_format(explanation.strip())
...
async def delete_question(
    ...
    try:
        ...
        logger.warning(f"[rpj] cleanup orphan question image failed: {e}")
...
async def reanalyze_question(
    ...
    async def reanalyze_async():
        from backend.modules.rpj.agents.intake.question_intake_agent import analyze_error, update_result
        ...
        try:
            ...
            state: QuestionIntakeState = {
                ...
                "subject": question.subject.value if question.subject else "chinese",
