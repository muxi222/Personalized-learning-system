...
async def rpj_ocr_process(state: RPJQuestionIntakeState) -> Dict[str, Any]:
    ...
    try:
        ...
        if pytesseract is None:
            # 学生模块环境可能未安装 pytesseract / tesseract，这里仅做降级，避免 import 阶段直接崩溃
            logger.warning("[RPJ OCR Process] pytesseract not installed; skipping OCR step (student TODO)")
            return {
                "ocr_result": "",
                "current_step": "rpj_ocr_process",
                "progress": 20.0,
...
class QuestionIntakeAgent(BaseAgent):
    ...
    async def process(
        ...
        **kwargs
        ...
        # 【输入】user_selected_subject=subject，text=raw_input + OCR(如有) 的题目文本/结构化题目
        # 【输出】对每题生成：
        ...
        #   }
        # 【实现建议】
        #   - OCR/文本提取后调用 settings.LLM_API_ENDPOINT 的 /chat/completions（二次判定+归一化）
