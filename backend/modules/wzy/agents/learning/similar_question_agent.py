...
class SimilarQuestionAgent(BaseAgent):
    ...
    async def process(
        ...
        **kwargs,
        ...
        """
        # 验证学科参数
        if subject and subject not in ['数学', '物理']:
            raise ValueError(f"WZY only supports math and physics, got {subject}")
            
        initial_state: SimilarQuestionState = {
            "question_id": question_id,
            "user_id": user_id,
            "task_id": task_id or f"similar_{question_id}",
            "retrieved_question_ids": [],
            "retrieved_questions": [],
            "similarity_scores": [],
            "errors": [],
            "student_profile": kwargs.get("student_profile") or {},
            "_subject": subject,  # 保存学科参数
        }
...
async def load_original_question(state: SimilarQuestionState) -> Dict[str, Any]:
    ...
    加载原题信息节点 - WZY版本
    """
    logger.info(f"[WZY][load_original] Loading question {state.get('question_id')}")
    ...
    try:
        async with async_session_maker() as session:
            ...
            # WZY只处理数学和物理学科
            subject_value = question.subject.value if question.subject else ""
            if subject_value not in ['数学', '物理']:
                return {
                    "errors": [f"WZY只支持数学和物理学科，原题为{subject_value}"],
                    "current_step": "load_original",
                    "progress": 10.0,
                }
        ...
        logger.error(f"[WZY][load_original] Error: {e}")
        return {
...
async def vector_retrieval(state: SimilarQuestionState) -> Dict[str, Any]:
    """
    向量检索节点 - RAG的Retrieval阶段 - WZY版本
    根据设计文档4.2节: 使用当前错题的向量进行相似度搜索
    """
    logger.info(f"[WZY][vector_retrieval] Searching similar questions")
    ...
    try:
        ...
        # 使用WZY的配置
        subject = state.get("subject") or original_question.get("subject") or "数学"
        ...
        if getattr(settings, "MCP_RETRIEVAL_ENABLED", False):
            ...
            # 过滤条件：用户ID和学科
            where_condition = {"user_id": user_id, "subject": subject} if user_id else {"subject": subject}
        ...
        if not getattr(settings, "MCP_RETRIEVAL_ENABLED", False):
            try:
                ...
                logger.debug(f"[WZY][vector_retrieval] GraphRAG expansion skipped: {_e}")

        logger.info(f"[WZY][vector_retrieval] Found {len(retrieved_ids)} similar questions")
        ...
        logger.error(f"[WZY][vector_retrieval] Error: {e}")
        return {
...
async def fetch_question_details(state: SimilarQuestionState) -> Dict[str, Any]:
    ...
    从结构化数据库查询检索到的题目完整信息
    """
    logger.info(f"[WZY][fetch_details] Fetching question details")
    ...
    try:
        ...
        logger.info(f"[WZY][fetch_details] Fetched {len(questions)} questions")
        ...
        logger.error(f"[WZY][fetch_details] Error: {e}")
        return {
...
async def generate_guidance(state: SimilarQuestionState) -> Dict[str, Any]:
    """
    生成引导文本节点 - RAG的Generation阶段 - WZY版本
    根据设计文档4.2节的Prompt设计
    """
    logger.info(f"[WZY][generate_guidance] Generating guidance text")
...
"""
    ...
    if personal.enabled:
        trace_id = f"wzy_similar_questions:{settings.MODULE_NAME}:{int(state.get('user_id') or 0)}:{int(state.get('question_id') or 0)}:{int(time.time())}"
        logger.info("[WZY][generate_guidance] using personal model=%s trace_id=%s", personal.default_model, trace_id)
