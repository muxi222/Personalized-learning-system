...
class QuestionIntakeAgent(BaseAgent):
    ...
    async def process(
        ...
        if subject and not self.validate_subject(subject):
            logger.error(f"Subject '{subject}' not supported by WZY module")  # 修改：TONY -> WZY
            return {
                "task_id": task_id,
                "question_id": None,
                "success": False,
                "errors": [f"Subject '{subject}' not supported by WZY module. Supported: {settings.SUBJECTS}"],  # 修改：TONY -> WZY
            }
...
async def semantic_parse(state: QuestionIntakeState) -> Dict[str, Any]:
    ...
    # TODO(student): 学科判定 + 分类归一化（统一模板 v1；留给学生实现）
    # 【输入】user_selected_subject=state.get("subject")，text=combined_input
    # 【输出】写回 structured_data：
    #   - chapter（从 CHAPTER_TAXONOMY 选 1 个）
    #   - knowledge_points（从 KNOWLEDGE_POINT_TAXONOMY 选 1~3 个）
    #   - tags（2~6 个短词）
    # 并额外输出 detected_subject/confidence 用于学科校验。
    # 【规则】detected_subject != user_selected_subject 且 confidence>=0.75：提示“学科不匹配”，拒绝入库/返回结果
    ...
    #   }
    # 【实现建议】调用 settings.LLM_API_ENDPOINT 的 /chat/completions；强模型优先，可用 WZY_HIGH_ACCURACY_MODEL 覆盖
