"""
错题录入Agent - Question Intake Agent (WZY版本)
基于TONY模块代码直接适配，仅修改模块名称和学科支持
"""
...
from backend.modules.wzy.config import settings  # 只改这一处
...
class QuestionIntakeAgent(BaseAgent):
    ...
    def __init__(self):
        super().__init__(subjects=settings.SUBJECTS)  # 使用WZY的SUBJECTS
    ...
    async def process(
        ...
        if subject and not self.validate_subject(subject):
            logger.error(f"Subject '{subject}' not supported by WZY module")  # 只改模块名
            return {
                "task_id": task_id,
                "question_id": None,
                "success": False,
                "errors": [f"Subject '{subject}' not supported by WZY module. Supported: {settings.SUBJECTS}"],  # 只改模块名
            }
...
async def save_to_database(state: QuestionIntakeState) -> Dict[str, Any]:
    ...
    try:
        ...
        subject_map = {
            ...
            # 其他学科可以保留或移除，但WZY只支持math和physics
        }
