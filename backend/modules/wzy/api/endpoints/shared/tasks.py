...
def _extract_image_id(task_result: Any) -> Optional[int]:
    ...
    注意：WZY模块只处理数学和物理学科，但任务结果结构应与Tony模块保持一致。
    """
...
async def get_task_status(
    ...
    注意：WZY模块只处理数学和物理学科，但任务状态查询是通用的。
    """
...
async def delete_task(
    ...
    DELETE /tasks/{task_id} 语义:
    - 如果任务处于 pending/processing 状态: 取消（标记为失败）（记录保留供后台工作程序使用）
    - 如果任务处于 completed/failed 状态: 硬删除任务记录
    
    注意：WZY模块只处理数学和物理学科，但任务取消/删除逻辑是通用的。
    """
    task = await crud_task.get_task_by_task_id(db, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
...
async def stream_task_status(
    ...
    注意：WZY模块只处理数学和物理学科，但任务流式推送是通用的。
