"""
Wzy "小书童" / Companion Chat API

Goals (Wzy-first, other modules will stub):
...
- Persist conversation math in DB for continuity ("digital twin" memory foundation).
...
from backend.modules.wzy.api.deps import get_current_user, get_db
...
async def _personal_model_should_run(svc, trace_id: str) -> bool:
    """
    In classroom deployments, students often forget to export PERSONAL_MODEL_ENABLED_WZY
    ...
    try:
        ...
        logger.warning("[%s] personal model auto-enabled (env flag missing): set PERSONAL_MODEL_ENABLED_WZY=true to suppress", trace_id)
        return True
...
async def _retrieve_context(
    *,
    ...
    1) Hybrid search (FAISS+BM25) over question corpus for wzy module
...
async def chat(
    ...
    if not await _personal_model_should_run(svc, trace_id):
        raise HTTPException(
            ...
            detail=(
                ...
                "  PERSONAL_MODEL_API_BASE_WZY=http://127.0.0.1:8005/v1\n"
                "  PERSONAL_MODEL_MODEL_WZY=wzy-dpo\n"
                "Then (re)start vLLM by: ./deploy/scripts/pipeline.sh serve-model --module wzy up\n"
    ...
    # Build messages for the model: system + short math + current user
    ...
    try:
        ...
        logger.info(
            ...
            trace_id,
            os.environ.get("PERSONAL_MODEL_API_BASE_WZY") or "http://127.0.0.1:8005/v1",
            svc.default_model,
            len(model_messages),
            len(retrieved),
        )
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "Try: curl http://127.0.0.1:8005/v1/models"
            ),
        )
...
async def chat_stream(
    ...
    # Retrieve context (Wzy-only) — do it before streaming starts.
    retrieved = await _retrieve_context(db=db, user_id=current_user.id, query=msg, profile=profile, top_k=5)
    retrieval_block = _format_retrieval_block(retrieved)
    try:
        src_counts: Dict[str, int] = {}
