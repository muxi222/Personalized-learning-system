...
from ..deps import get_current_user, get_current_user_id, get_optional_user_id, get_db, oauth2_scheme
from ....services.gemini_ocr_service import (
...
async def analyze_exam_image(
    ...
    # 获取或创建原始图片记录
    from ....db.models import ImageFileTypeEnum
    ...
    if existing_image:
        ...
        if not relative_path.startswith("data/uploads"):
            relative_path = f"./data/uploads/{os.path.basename(upload_dir)}/corrections/{subject_en}/{filename}"
    ...
    try:
        ...
        if correction_result.success and correction_result.corrected_image_base64:
            ...
            if existing_corrected_image:
                ...
                corrected_path = os.path.join(
                    get_user_upload_dir(UPLOAD_DIR, current_user, "corrections", result.subject.value),
                    corrected_filename
                )
                ...
                logger.info(f"Saved corrected image: hash={corrected_file_hash[:16]}..., path={corrected_path}, id={corrected_image_file.id}")
            
            # 生成URL
            corrected_image_url = get_user_file_url(current_user, "corrections", result.subject.value, get_filename_from_path(corrected_path))
            logger.info(f"Corrected image URL: {corrected_image_url}")

        # 保存批注记录到数据库
        from ....db.models import ExamCorrection, QuestionSourceEnum
        
        exam_correction = ExamCorrection(
            user_id=current_user.id,
            subject=result.subject,
            grade=grade or result.grade,
            exam_title=hint or f"{result.subject.value}试卷批改",
        ...
        # 为错误的题目自动创建错题记录
        from ....crud import crud_question
        ...
        for q in result.questions:
            if not q.is_correct:  # 只为错题创建记录
                ...
                question_image_path = os.path.join(
                    get_user_upload_dir(UPLOAD_DIR, current_user, "questions", result.subject.value),
                    question_filename
                )
                ...
                # 生成URL路径
                question_image_url = get_user_file_url(current_user, "questions", result.subject.value, question_filename)
...
async def get_user_image(
    ...
    from ....core.file_utils import get_user_directory_name
...
async def get_correction_image(
    ...
    """
    from fastapi.responses import FileResponse
    from ....crud import crud_exam_correction, crud_image_file
    from ....core.config import settings
    from jose import jwt, JWTError
    import os
