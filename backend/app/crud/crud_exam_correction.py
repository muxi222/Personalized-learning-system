...
async def get_correction_statistics(
    ...
    ).group_by(ExamCorrection.subject)
    ...
    if period == "week":
        ...
        ).group_by(func.date(ExamCorrection.created_at)).order_by(func.date(ExamCorrection.created_at))
    else:
        # 按周统计
        time_query = select(
        ...
        ).group_by(extract('week', ExamCorrection.created_at)).order_by(extract('week', ExamCorrection.created_at))
