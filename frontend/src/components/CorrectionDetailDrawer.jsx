...
export default function CorrectionDetailDrawer({
  ...
  return (
    <>
      ...
      {/* 抽屉内容 */}
      <div className="fixed right-0 top-0 bottom-0 w-full lg:w-4/5 xl:w-3/4 bg-white z-50 overflow-y-auto shadow-2xl animate-slide-in-right">
        {/* 头部工具栏 */}
        <div className="sticky top-0 bg-white backdrop-blur-xl border-b border-slate-200 z-10">
              ...
              {allIds.length > 1 && (
                <div className="flex items-center gap-2 ml-4">
                  <button
                    onClick={handlePrev}
                    disabled={!hasPrev}
                    className="p-2 hover:bg-slate-50 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                    title="上一个 (←)"
                  >
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              <div className={`p-4 rounded-lg ${scorePercent >= 60 ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-red-500/10 border border-red-500/30'}`}>
                <div className="text-slate-600 text-sm mb-1">总得分</div>
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-bold text-slate-800">{correction.total_score}</span>
                  <span className="text-slate-500">/ {correction.max_score}</span>
                </div>
              </div>

              <div className="p-4 rounded-lg bg-slate-50">
                <div className="text-slate-600 text-sm mb-1">正确率</div>
              ...
              <div className="p-4 rounded-lg bg-emerald-500/10">
                <div className="text-emerald-600 text-sm mb-1">答对</div>
                <div className="text-2xl font-bold text-emerald-600">{correction.correct_count}</div>
              </div>

              <div className="p-4 rounded-lg bg-red-500/10">
                <div className="text-red-600 text-sm mb-1">答错</div>
                <div className="text-2xl font-bold text-red-600">{correction.wrong_count}</div>
              </div>
            </div>
                      ...
                      {questions.map((q, index) => (
                        ...
                        >
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <span className={clsx("text-lg font-bold", q.is_correct ? "text-emerald-600" : "text-red-600")}>
                              {q.score}/{q.max_score}
                            </span>
                          </div>
                          ...
                          {!q.is_correct && (
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              <div className="bg-red-500/10 border border-red-500/30 rounded p-2">
                                <div className="text-red-600 mb-1">你的答案:</div>
                                <div className="text-slate-600">{q.student_answer || '未作答'}</div>
                              </div>
                              {q.correct_answer && (
                                <div className="bg-emerald-500/10 border border-emerald-500/30 rounded p-2">
                                  <div className="text-emerald-600 mb-1">正确答案:</div>
                                  <div className="text-slate-600">{q.correct_answer}</div>
                                </div>
                              )}
                            </div>
                          )}
                    ...
                    <div className="space-y-2">
                      {correction.weak_points.map((point, index) => (
                        <div key={index} className="flex items-center gap-2 p-3 bg-amber-500/10 rounded-lg">
                          <span className="w-6 h-6 bg-amber-500/30 text-amber-600 rounded-full flex items-center justify-center text-xs font-bold">
                            {index + 1}
                          </span>
